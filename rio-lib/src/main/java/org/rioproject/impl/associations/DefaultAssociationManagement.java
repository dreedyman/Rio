 ...
 * @author Dennis Reedy
 */
public class DefaultAssociationManagement implements AssociationManagement {
    /**
     ...
     * DefaultAssociationManagement object will unadvertise the ServiceBean using the
     * ServiceBean instance's ServiceBeanControl object. If false, the
     * DefaultAssociationManagement object will not unadvertise the ServiceBean.
     */
     ...
     * The Logger.
     */
    private static final Logger logger = LoggerFactory.getLogger(DefaultAssociationManagement.class);
    /**
    ...
    /**
     * Create an DefaultAssociationManagement instance. Uses the current thread's context
     * class loader to provide the caller/client with properly annotated
     * proxies for associated services
     *
     */
    public DefaultAssociationManagement() {
        this(null);
    }

    /**
     * Create an DefaultAssociationManagement instance. Uses the specified class loader
     ...
     */
    public DefaultAssociationManagement(final ClassLoader cl) {
        if(cl==null) {
    ...
    /**
     * @see org.rioproject.associations.AssociationManagement#setServiceBeanControl
     */
    ...
    public void terminate() {
        ...
        callerCL = null;
        control = null;
        logger.debug("Terminated DefaultAssociationManagement for {}", clientName);
    }
     ...
     *
     * @param context The ServiceBeanContext object for the ServiceBean
     */
        ...
        if(newDesc != null) {
            if(!update) {
                if(logger.isTraceEnabled()) {
                    ...
                    sb.append(" DefaultAssociationManagement descriptors=").append(newDesc.length);
                    if(newDesc.length>0)
                        sb.append("\n");
    ...
    class Listener implements AssociationListener {
        private final List<Association> requiredAssociations = new CopyOnWriteArrayList<>();
        ...
        void checkAdvertise() {
            ...
            if((requiredAssociations.size() >= numRequires.get()) && !advertised.get()) {
                if(context!=null) {
                    ...
                    logger.debug("DefaultAssociationManagement created without ServiceBeanContext, unable to verify service {} " +
    ...
    public class AssociationHandler extends ServiceDiscoveryAdapter implements FaultDetectionListener<ServiceID>,
        ...
        /**
         * @see org.rioproject.impl.fdh.FaultDetectionListener#serviceFailure
         */
        @SuppressWarnings("unchecked")
                ...
                if (aHandler.associationDescriptor.matchOnName()) {
                    ...
                    if (service instanceof Administrable) {
                        ...
                        if (adminObject instanceof JoinAdmin) {
                            ...
                            hailMaryCheck = true;
                            logger.debug("DefaultAssociationManagement for [{}], associated service proxy " +
                                         "for colocated service does not implement {}, using declared service name",
                                         clientName, JoinAdmin.class.getName());
                        }
                    } else {
                        hailMaryCheck = true;
                        logger.debug("DefaultAssociationManagement for [{}], associated service proxy for" +
