...
public class ServiceBeanActivation {
     ...
     * ServiceBean to use when obtaining configuration attributes
     * @param defaultServiceName The default name of the service
     * @param configArgs Configuration for the ServiceBean
     * @param loader The ClassLoader which loaded the ServiceBean
     *
     * @return A ServiceBeanContext. A new ServiceBeanCOntext is created each
     * time
     *
    ...
    throws Exception {
        ...
        configParms.putAll(readServiceBeanConfig(configComponent,
                                                 defaultServiceName,
                                                 config));

        String jmxName = (String)config.getEntry(configComponent,
                                                 "jmxName",
                                                 String.class,
                                                 null);
        if(jmxName!=null)
        ...
        if(exportCodebase.indexOf(".jar") != -1) {
            ...
            throw new RuntimeException("Cannot determine export codebase " +
                                       "from "+exportCodebase);
        }
        ...
        defaultFDHBundle.addMethod("setConfiguration",
         ...
         * Since the system threshold is he summation of all depletion oriented
         * resources, total utilization is =
         * (num_available_processors * cpu_utilization *
         *  mem_utilization * diskspace_utilization) */
        double defaultSystemThreshold =
        ...
        sla.getSystemRequirements()
            .addSystemThreshold(SystemRequirements.SYSTEM,
        ...
        JSBContext jsbContext = new JSBContext(sElem,
                                               jsbManager,
                                               computeResource,
                                               null); /* Shared Configuration */
    ...
    throws Exception {
        ...
        /* Set the ClassLoader for the DiscoveryManagementZool to be the
        ...
        String serviceName = (String)config.getEntry(configComponent,
                                                     "serviceName",
                                                     String.class,
                                                     defaultServiceName);
        configParms.put(ServiceBeanConfig.NAME, serviceName);
        /* Get the comment for the ServiceBean */
        String serviceComment = (String)config.getEntry(configComponent,
                                                        "serviceComment",
                                                        String.class,
                                                        null);
        ...
        String opStringName = (String)config.getEntry(configComponent,
                                                      "opStringName",
                                                      String.class,
                                                      BOOT_OPSTRING);
        ...
        LookupLocator[] lookupLocators =
            (LookupLocator[])config.getEntry(configComponent,
                                             "initialLookupLocators",
                                             LookupLocator[].class,
                                             null);
        /* Get LoggerConfig values */
        LoggerConfig[] logConfigs =
            (LoggerConfig[])config.getEntry(configComponent,
                                            "loggerConfigs",
                                            LoggerConfig[].class,
                                            null);
        if(logConfigs != null) {
     ...
     * @throws Exception If there are errors
     */
    static void checkUtilityConfiguration(Configuration config) throws Exception {
        if(config == null)
            throw new NullPointerException("configArgs are null");

        Boolean multiCheck = (Boolean)config.getEntry(BOOT_COMPONENT,
                                                      "verifyMulticast",
                                                      Boolean.class,
                                                      false);
        ...
        if(multiCheck) {
            ...
            if(logger.isLoggable(Level.FINE))
                logger.fine("Multicast responds okay");
        }
        /* Check for the creation of a ShutdownHook */
        Boolean createShutdownHook = (Boolean)config.getEntry(BOOT_COMPONENT,
                                                              "createShutdownHook",
                                                              Boolean.class,
                                                              true);
        if(logger.isLoggable(Level.FINE))
    ...
    public static class LifeCycleManager implements DiscardManager, LifeCycle {
        ...
        Map<DestroyAdmin, Subject>registrationMap =
                                          new HashMap<DestroyAdmin, Subject>();

        /*
         * Register the ServiceBean to the LifeCycleManager and advertise it
         */
        @SuppressWarnings("unchecked")
        public void register(Object sbProxy, ServiceBeanContext context)
            ...
            try {
                ...
                try {
                    ProxyPreparer prep =
                    (ProxyPreparer)context.getConfiguration().
                                           getEntry(BOOT_COMPONENT,
                                                    "adminProxyPreparer",
                    ...
                    if(sbProxy instanceof Administrable) {
                        ...
                        if(adminObject instanceof DestroyAdmin) {
                            ...
                            registrationMap.put((DestroyAdmin)adminObject,
                                                subject);
                        }
                    }
                } catch (Exception e) {
                ...
                String name = (context == null ? "unknown name" :
                                             context.getServiceElement().getName());
                throw new JSBControlException("ServiceBean ["
                                              + name
                                              + "] advertise failed", t);
            }
        }
