 ...
 * with a {@link org.rioproject.core.ServiceElement}
 *
 * @author Dennis Reedy
 */
public class ServiceElementUtil {
    ...
    private static Logger logger = 
        Logger.getLogger("org.rioproject.jsb.ServiceElementUtil");
           ...
           sElem.getComponentBundle().getCodebase().startsWith("http")) {
            ...
            if(sElem.getExportBundles().length>0) {
                if(sElem.getExportBundles()[0].getCodebase()!=null)
                    httpCodebase =
                        sElem.getExportBundles()[0].getCodebase().startsWith("http");
            }
        }
        return(httpCodebase);
    }
        ...
        if(loggerConfigs1!=null && loggerConfigs2!=null) {
            for (LoggerConfig aLoggerConfigs2 : loggerConfigs2) {
                if (LoggerConfig.isNewLogger(aLoggerConfigs2, loggerConfigs1)) {
                    different = true;
                    break;
                } else if (LoggerConfig.levelChanged(aLoggerConfigs2,
                                                     loggerConfigs1)) {
                    different = true;
                    break;
                }
    ...
    public static boolean hasDifferentServiceUIs(Entry[] serviceUIs, 
        ...
        try {
            Configuration config2 =
                ConfigurationProvider.getInstance(sElem2.getServiceBeanConfig().
                                                          getConfigArgs());
    ...
    public static boolean hasDifferentGroups(ServiceElement sElem1, 
        ...
        if (groups1 == DiscoveryGroupManagement.ALL_GROUPS &&
            groups2 == DiscoveryGroupManagement.ALL_GROUPS)
            return false;
    ...
    public static ServiceElement copyServiceElement(ServiceElement sElem) {
        ...
        ServiceElement elem =
            new ServiceElement(sElem.getProvisionType(),
                               sbc,
     ...
     *
     */
    public static PlatformCapability[]
     ...
     */
    public static AssociationDescriptor[]
        getAssociationDescriptors(ServiceElement elem, AssociationType type) {
    ...
    public static String formatDiscoverySettings(ServiceBeanConfig sbConfig) {
        ...
        if(locs != null && locs.length > 0) {
            for(int i=0; i<locs.length; i++) {
                if(i>0)
                    buff.append(", ");
                buff.append("jini://"+locs[i].getHost()+":"+locs[i].getPort());
            }
        }
