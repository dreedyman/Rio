...
/**
 * Handles the parsing for the Organization element
 *
 * @author Jerome Bernard
 */
class OrganizationHandler implements Handler {
    public parse(Object element, Object options) {
        ...
        if(!((parent.name() == "ServiceBean" || parent.name() == "SpringBean") && sDescriptor == null)) {
            String organization = element.text()
            if (sDescriptor)
                sDescriptor.organization = organization
            else
