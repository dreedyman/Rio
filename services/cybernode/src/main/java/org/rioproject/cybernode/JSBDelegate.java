...
public class JSBDelegate implements ServiceBeanDelegate {
    ...
    public void advertise() throws JSBControlException {
        if(execManager!=null)
            return;
        if(serviceProxy==null) {
            throw new JSBControlException("Cannot advertise " +
                                          "["+sElem.getName()+"], "+
                                          "Proxy is null");
        }
        if(terminated || terminating)
            throw new JSBControlException("advertising service "+
     ...
     *
     * @throws JSBInstantiationException if there are errors creating the
     * service bean
     */
    ...
    throws JSBInstantiationException {
        ...
        */
        ...
        if(abortThrowable != null) {
            ...
            JSBInstantiationException toThrow;
            if(abortThrowable instanceof JSBInstantiationException) {
                toThrow = (JSBInstantiationException) abortThrowable;
            } else {
                JSBInstantiationException.ExceptionDescriptor exDesc =
                    new JSBInstantiationException.ExceptionDescriptor(abortThrowable.getClass().getName(),
                                                                      abortThrowable.getLocalizedMessage(),
                                                                      abortThrowable.getStackTrace());
                toThrow = new JSBInstantiationException("ServiceBean ["+sElem.getName()+"] "+
