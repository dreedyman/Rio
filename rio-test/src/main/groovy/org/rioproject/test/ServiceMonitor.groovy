...
/**
 * The class represents a Jini service monitor. Using this class, you
...
class ServiceMonitor {
    ...
    private static Logger logger = Logger.getLogger(ServiceMonitor.class.getPackage().name)
     ...
     *             services.
     * @param type The type to monitor
     */
    ServiceMonitor(ServiceDiscoveryManager sdm, Class type) {
        this.sdm = sdm
        this.type = type
    }
     ...
     *          timeout has elapsed).
     */
    def waitFor(long serviceCount) throws TimeoutException {
        logger.info "Waiting for [${serviceCount}] services of type [${type.name}] ..."
       ...
       logger.info "Waiting for [${serviceCount}] services of type [${type.name}] - OK"
    }
    ...
    private int getAndLogCount(long expected) {
        logger.info("${type.name} found: [${getCount()}], Expecting [${expected}]")
        return getCount()
    }
    ...
    public List getServices() {
        ServiceTemplate template = new ServiceTemplate(null, (Class[])type, null)
        ServiceItem[] items = sdm.lookup(template, Integer.MAX_VALUE, null)
        List services = new ArrayList()
        for (ServiceItem item : items) {
            services.add(item.service) // TODO: prepare
        }
        return services
    }    
