...
deployment(name: 'Tomcat Deploy') {
    ...
    serviceExec(name: 'Tomcat') {
        ...
        software(name: 'Tomcat', version: '9.0.36', removeOnDestroy: true) {
            install source: 'https://downloads.apache.org/tomcat/tomcat-9/v9.0.52/bin/apache-tomcat-9.0.52.zip',
                    target: 'tomcat',
                    unarchive: true
            postInstall(removeOnCompletion: false) {
                if (!System.properties['os.name'].toLowerCase().contains('windows')) {
                    execute command: '/bin/chmod +x ${rio.home}/system/external/tomcat/apache-tomcat-9.0.44/bin/*sh'
                }
            }
        }
